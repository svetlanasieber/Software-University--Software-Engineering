...
public class ControllerImpl implements Controller {
    ...
    public String addMagician(String type, String username, int health, int protection, String magicName) {
        ...
        switch (type) {
            case "Wizard":
                if (isInvalidName(magicName)) {
                    throw new NullPointerException(MAGIC_CANNOT_BE_FOUND);
                }
                magician = new Wizard(username, health, protection, this.magics.findByName(magicName));
                break;
            case "BlackWidow":
                if (isInvalidName(magicName)) {
                    throw new NullPointerException(MAGIC_CANNOT_BE_FOUND);
                }
                magician = new BlackWidow(username, health, protection, this.magics.findByName(magicName));
                break;
            default:
    ...
    private boolean isInvalidName(String magicName) {
        return this.magics.findByName(magicName) == null;
