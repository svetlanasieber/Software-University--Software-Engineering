...
describe("Lock", function () {
  ...
  describe("Demo functions", function () {
    it("should revert custom error", async function () {
      const { lock } = await loadFixture(deployOneYearLockFixture);

      await expect(lock.testCustomError())
        .to.be.revertedWithCustomError(lock, "LockReverted")
        .withArgs(10);
    });
  });
});
...
describe("Demo from Lecture", function () {
  it("Should update block time", async function () {
    ...
    const newBlockHeight = await ethers.provider.getBlockNumber();
  ...
  it("should check hex", async function () {
